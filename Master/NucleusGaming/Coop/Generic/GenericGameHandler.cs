                    ...
                    if (!gen.ThirdPartyLaunch)
                    {
                        ...
                        if (/*context.KillMutex?.Length > 0 || */(gen.HookInit || (gen.RenameNotKillMutex && context.KillMutex?.Length > 0) || gen.SetWindowHookStart || gen.BlockRawInput || gen.CreateSingleDeviceFile) && !gen.CMDLaunch && !gen.UseForceBindIP && !gen.LaunchAsDifferentUsers && !gen.LaunchAsDifferentUsersAlt) /*|| (gen.CMDLaunch && i==0))*/
                        {
                            ...
                            uint sguOutPID = StartGameUtil.StartGame(/*
                                ...
                                , player.MonitorBounds.Y, DocumentsRoot/*, gen.RunAsAdmin, rawHID,*/ /*GetRelativePath(linkFolder, nucleusRootFolder)*/);
                ...
                if ((proc != null && !Process.GetProcesses().Any(x => x.Id == proc.Id)) || gen.ForceProcessSearch || gen.NeedsSteamEmulation || gen.ForceProcessPick || proc == null || gen.CMDLaunch || gen.UseForceBindIP || gen.GameName == "Halo Custom Edition" || (proc != null && !IsRunning(proc)) /*|| gen.LauncherExe?.Length > 0*/)
                {
                    ...
                    if (!gen.ForceProcessPick)
                    {
                        ...
                        for (int times = 0; times < 200; times++)
                        {
                            ...
                            if (gen.ChangeExe)
                            {
                                proceName = Path.GetFileNameWithoutExtension(this.userGame.Game.ExecutableName) + " - Player " + (i + 1) + ".exe";
                            }                          
        ...
        private void ChangeExe(int i)
        {
            ...
            if (File.Exists(Path.Combine(instanceExeFolder, newExe)))
            {
                ...
                else
                {
                    if (File.Exists(Path.Combine(instanceExeFolder, this.userGame.Game.ExecutableName)))
                    {
                        File.Delete(Path.Combine(instanceExeFolder, this.userGame.Game.ExecutableName));
                    }
                }
            }
